...
describe('Tagtype APIs should handle ', function () {
    ...
    it('create a null tagtype', function (done) {
        data.set('tagtype', null)
    ...
    it('get a nonexistent tagtype', function (done) {
        centralApi.delete('/api/tagtype/' + uuid())
            .set('Authorization', 'Bearer ' + data.get('authorizedToken'))
            .end(function (err, res) {
                // console.log(res.body)
                expect(res.status).to.equal(200)
    ...
    it('delete a nonexistent tagtype', function (done) {
        centralApi.delete('/api/tagtype/' + uuid())
            .set('Authorization', 'Bearer ' + data.get('authorizedToken'))
            .end(function (err, res) {
                // console.log(res.body)
