...
def main(argv):
    ...
    if options.refresh_token:
        ...
        TestImapAuthentication(
            options.user,
            GenerateOAuth2String(options.user,
...
def main_bd():
    ...
    #  --client_secret=VWFn8LIKAMC-MsjBMhJeOplZ \
    #  --generate_oauth2_token
    cred_filename = 'configs/bd_config.json'
    ...
    with open(cred_filename, 'r') as fp:
        d = json.load(fp)
        d['refresh_token'] = response['refresh_token']
        d['access_token'] = response['access_token']
    with open(cred_filename, 'w') as fp:
