...
def run_writer(sensor, email, cid, ckey, freq):
  global elapsed_time_write
  global num_writes
  global num_violation
  global num_timeout
  global num_server_errors
  global elapsed_time_timeout
  global elapsed_time_violation
  global num_unknown
  global num_corruption
...
sensors = [ "sensor_"+ str(x) for x in range (1,100)] # sensor_X, X means the place or sensors in Sensor table (somewhat weird to me.. should be fixed)
sensor = sensors[0]
...
while True:
  time.sleep(5)
  print "Number of successful write : " + str(num_writes)
  ...
  print "---------------------------------------"
  print "Number of successful read : " + str(num_reads)
  if num_reads != 0:
