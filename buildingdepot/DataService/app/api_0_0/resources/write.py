...
class Write(Resource):
    ...
    def post(self, sensor_name):
        ...
        if permission(g.user, sensor_name) != 'r/w':
            return not_allowed(
                'You do not have write permission to sensor {}'.format(
                    sensor_name))
        # permission_e = time()
        parser = reqparse.RequestParser()
        parser.add_argument('timeseries',
                            type=timeseries_validator,
                            required=True,
                            location='json')
        args = parser.parse_args()

        points = [[int(pair.keys()[0]), pair.values()[0]]
        ...
        for email in emails:
            pipe.set('latest_point:{}:{}'.format(sensor_name, email),
