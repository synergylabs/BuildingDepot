...
class Write(Resource):
    ...
    def get(self, sensor_name):
        ...
        query = 'select value from {} where time > {}s'.format(sensor_name, args['start'])
    ...
    def post(self, sensor_name):
        ...
        if permission(g.user, sensor_name) != 'r/w':
            return not_allowed('You do not have write permission to sensor {}'.format(sensor_name))
        # permission_e = time()
        parser = reqparse.RequestParser()
        ...
        for email in emails:
            pipe.set('latest_point:{}:{}'.format(sensor_name, email), '{}-{}'.format(max_point[0], max_point[1]))
        pipe.execute()
        # sub_e = time()
