...
class SensorViewService(MethodView):
    ...
    def get(self, name):
        ...
        response.update(
            {"views_owned": all_views, "available_fields": available_fields}
        )
    ...
    def post(self, name):
        ...
        if r.get("parent:{}".format(name)):
            return jsonify(
                {"success": "False", "error": "Sensor views can't have sub-views."}
            )
        ...
        field_tags = [
            {"name": "field", "value": field.strip()} for field in fields_list
        ]
        available_tags = Building.objects(name=building).first().tags
        available_tags = [
            {"name": tag.name, "value": tag.value} for tag in available_tags
        ]
        tags = (
            tags
            + [{"name": tag.name, "value": tag.value} for tag in sensor.tags]
            + [{"name": "parent", "value": sensor.name}]
            + field_tags
        )
        if any(tag not in available_tags for tag in tags):
            missing_tags = set([(tag["name"], tag["value"]) for tag in tags]) - set(
                [(tag["name"], tag["value"]) for tag in available_tags]
            )
            missing_tags = [
                {"name": name, "value": value} for name, value in missing_tags
            ]
            return jsonify(
                {
                    "success": "False",
                    "building_tags_required": missing_tags,
                    "error": "Cannot create "
                    "sensor views "
                    "without the "
                    "required building "
