    ...
    def get(self):
        ...
        if user_groups is not None:
            ...
            if permissions is not None:
                ...
                sensor_groups = SensorGroup._get_collection().find(
                    {"name": {"$in": sensor_group_names}}
                )

                if sensor_groups is not None:
                    for sensor_group in sensor_groups:
                        sensor = Sensor._get_collection().find(
                            {"tags": sensor_group.tags}
                        )
                        ...
                        if sensor is not None:
                            ...
                            metadata = Sensor._get_collection().find(
                                {"name": sensor["name"]}, {"metadata": 1, "_id": 0}
                            )[0]["metadata"]
                            metadata = [
                                {"name": key, "value": val}
