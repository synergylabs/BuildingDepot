...
def tagtype_delete():
    ...
    res = requests.delete(
        request.url_root + "api/tagtype/" + name, headers=session["headers"]
    ).json()
    if res["success"] == "False":
        flash(res["error"])
    return redirect(url_for("central.tagtype"))
...
def buildingtemplate_delete():
    name = request.form.get("name")
    res = requests.delete(
        request.url_root + "api/template/" + name, headers=session["headers"]
    ).json()
    if res["success"] == "False":
        flash(res["error"])
    return redirect(url_for("central.buildingtemplate"))
...
def building_delete():
    name = request.form.get("name")
    res = requests.delete(
        request.url_root + "api/building/" + name, headers=session["headers"]
    ).json()
    if res["success"] == "False":
        flash(res["error"])
    return redirect(url_for("central.building"))
...
def building_metadata(name):
    ...
    if request.method == "GET":
        metadata = Building._get_collection().find(
...
def building_tags_delete(building_name):
    ...
    res = requests.delete(
        request.url_root + "api/building/" + building_name + "/tags",
        ...
        triples = [
            {
                ...
                "tags": [
                    {"name": elem.name, "value": elem.value} for elem in item.tags
                ],
            }
...
def dataservice():
    ...
    if form.validate_on_submit():
        ...
        res = requests.post(
            request.url_root + "api/dataservice",
            data=json.dumps(payload),
            headers=session["headers"],
        ).json()
        if res["success"] == "False":
...
def dataservice_buildings(name):
    ...
    if request.method == "GET":
        buildings = DataService._get_collection().find(
            {"name": name}, {"buildings": 1, "_id": 0}
        )[0]["buildings"]
...
def dataservice_admins(name):
    ...
    if request.method == "GET":
        admins = DataService._get_collection().find(
            {"name": name}, {"admins": 1, "_id": 0}
        )[0]["admins"]
...
def dataservice_delete():
    name = request.form.get("name")
    res = requests.delete(
        request.url_root + "api/dataservice/" + name, headers=session["headers"]
    ).json()
    if res["success"] == "False":
        flash(res["error"])
    return redirect(url_for("central.dataservice"))
...
def permission_create():
    ...
    form.user_group.choices = sorted(
        [(obj.name, obj.name) for obj in UserGroup.objects]
    )
    form.sensor_group.choices = sorted(
        [(obj.name, obj.name) for obj in SensorGroup.objects]
    )
    if form.validate_on_submit():
        # Doesn't allow duplicate permissions
        if (
            Permission.objects(
                user_group=form.user_group.data, sensor_group=form.sensor_group.data
            ).first()
            is not None
        ):
            flash(
                "There is already permission pair {} - {} specified".format(
                    form.user_group.data, form.sensor_group.data
                )
            )
            return redirect(url_for("central.permission"))
        if defs.create_permission(
            form.user_group.data,
            ...
            if permission_allowed(form.sensor_group.data, session["email"]):
                Permission(
                    user_group=str(form.user_group.data),
                ...
                flash(
                    "Not authorized to create permissions for the specified SensorGroup tags"
                )
        else:
            flash("Unable to communicate with the DataService")
        return redirect(url_for("central.permission"))
