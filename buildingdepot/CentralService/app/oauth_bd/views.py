...
def generate_credentials():
    ...
    if user is not None and user.first_login and user.verify_password(password):
        ...
        response.update(
            {
                "login": "You have logged in for the first time.Please change your\
         password on the UI"
            }
        )
    ...
    if user is not None and user.verify_password(password):
        ...
        if creds_size > 0:
            client = Client.objects(
                client_id=creds[creds_size - 1]["client_id"],
                client_secret=creds[creds_size - 1]["client_secret"],
            ).first()
            if client != None:
                # Set token expiry period and create it
                expires = datetime.utcnow() + timedelta(seconds=expires_in)
            ...
            client = Client.objects(
                client_id=keys[0]["client_id"], client_secret=keys[0]["client_secret"]
            ).first()
            if client != None:
                # Set token expiry period and create it
                expires = datetime.utcnow() + timedelta(seconds=expires_in)
